...
SECONDS_4 = 100
SECONDS_5 = 160
THRESHOLD_1 = -3.9  # -3.9 is distance from inclined start
#THRESHOLD_1 = -5.15
THRESHOLD_2 = -3.45
#THRESHOLD_2 = -4.7  # -4.35 is distance from rugged start
THRESHOLD_3 = -4.15
THRESHOLD_4 = -3.9

# Positions
SPAWN_POS = [-0.8, 0, 0.1]
RUGGED_POS = [-0.5, 0, 0.1]
INCLINED_POS = [0.5, 0, 0.1]
SPAWN_POSITIONS = [SPAWN_POS, RUGGED_POS, INCLINED_POS]
...
def decode_genome(genome):
	...
	w1, w2, w3, model, data, tracker, brain_genome = decode_brain_genome(robot_graph, genome)
	...
	return ctrl, model, data, robot_graph, tracker, w1, w2, w3
...
def get_io_sizes_for_graph(robot_graph) -> tuple[int, int]:
	...
	world.spawn(core.spec, position=random.choice(SPAWN_POSITIONS))
...
def decode_brain_genome(robot_graph, genome):
	...
	w1, w2, w3 = extract_weights(brain_genome, l1, l2, l3, input_size, output_size)

	return w1, w2, w3, model, data, tracker, brain_genome

def extract_weights(brain_genome, l1, l2, l3, input_size, output_size):
...
def main() -> None:
	...
	seconds = SECONDS_1
	save_path_graph = DATA / f"best_body_robot_graph_{run_name}.json"
	save_path_w1 = DATA / f"best_brain_w1_{run_name}.npy"
	save_path_w2 = DATA / f"best_brain_w2_{run_name}.npy"
	save_path_w3 = DATA / f"best_brain_w3_{run_name}.npy"
	print(f"Save path of graph: {save_path_graph}")
	...
	for g in range(GENERATIONS):
		...
		for candidate in population:
			ctrl, model, data, robot_graph, tracker, w1, w2, w3 = decode_genome(candidate)

			# Telling MuJoCo to call our controller every timestep
			mj.set_mjcb_control(lambda m, d: ctrl.set_control(m, d))

			# Dynamic simulation duration
			if seconds == SECONDS_4 and best_f_overall > THRESHOLD_4:
				seconds = SECONDS_5
			elif seconds == SECONDS_3 and best_f_overall > THRESHOLD_3:
				seconds = SECONDS_4
			elif seconds == SECONDS_2 and best_f_overall > THRESHOLD_2:
				seconds = SECONDS_3
			elif seconds == SECONDS_1 and best_f_overall > THRESHOLD_1:
				seconds = SECONDS_2
			...
			if f > best_f_overall:
				...
				# Save the best robot_graph that will be used to rebuild the body
				save_graph_as_json(best_body_graph, save_path_graph)
				
				# Save brain genome (i.e., the flat vector) for rebuilding controller
				np.save(save_path_w1, best_brain_w1)
				np.save(save_path_w2, best_brain_w2)
				np.save(save_path_w3, best_brain_w3)
